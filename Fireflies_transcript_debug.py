...
def fetch_transcripts(participant_email, from_date=None, to_date=None):
    ...
    print(f"Fetching transcripts for {participant_email} from {from_date} to {to_date}")  # Debug log
    print(f"Query variables: {variables}")  # Debug log

    response = requests.post(BASE_URL, json={"query": query, "variables": variables}, headers=HEADERS)
    try:
        response_data = response.json()
        print(f"Transcripts API Response: {response_data}")  # Debug log

        if response.status_code == 200 and "data" in response_data:
            return response_data["data"]["transcripts"]
        else:
            raise Exception(f"Error fetching transcripts: {response_data.get('errors')}")
    except Exception as e:
        print(f"Error parsing transcripts response: {response.text}")  # Debug log
        raise e
...
def fetch_transcript_details(transcript_id):
    ...
    print(f"Fetching details for transcript ID: {transcript_id}")  # Debug log
    print(f"Request payload: {{'query': {query}, 'variables': {variables}}}")  # Debug log

    response = requests.post(BASE_URL, json={"query": query, "variables": variables}, headers=HEADERS)
    try:
        response_data = response.json()
        print(f"Transcript Details API Response: {response_data}")  # Debug log

        if response.status_code == 200 and "data" in response_data:
            return response_data["data"]["transcript"]
        else:
            raise Exception(f"Error fetching transcript details: {response_data.get('errors')}")
    except Exception as e:
        print(f"Error parsing transcript details response: {response.text}")  # Debug log
        raise e
...
def main():
    ...
    try:
        ...
        print(f"Fetched transcripts: {transcripts}")  # Debug log
        if transcripts:
